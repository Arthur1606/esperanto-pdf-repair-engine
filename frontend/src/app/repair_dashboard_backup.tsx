...
export default function Home() {
  ...
  const handleBatchUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadDebug((prev: any) => ({ ...prev, status: "Subiendo lote al backend..." }));
    try {
      ...
      const data = await res.json();
      setUploadDebug((prev: any) => ({ ...prev, status: "Lote procesando en auditor", docCount: data.length }));
      ...
      const checkBatchReport = async () => {
        ...
        if (reportData.processed_documents >= reportData.total_documents) {
          setLoading(false);
          setUploadDebug((prev: any) => ({ ...prev, status: "Auditoría masiva completada" }));
        } else {
      ...
      setUploadDebug((prev: any) => ({ ...prev, status: "Error", error: (err as Error).message }));
      setLoading(false);
    }
  };
  ...
  const handleToggleApproval = (word: string) => {
    setApprovedCorrections((prev: any) => ({
      ...prev,
      [word]: !prev[word]
    }));
  };
